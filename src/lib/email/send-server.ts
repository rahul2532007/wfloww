// Server-only transactional email helper. Delivery is intentionally best-effort:
// a missing Resend configuration or a provider failure never blocks an app mutation.
import { Resend } from "resend";

export interface SendTxOptions {
  templateName: "booking-confirmation" | "doctor-invite";
...
function bookingConfirmation(data: Record<string, unknown>): RenderedEmail {
  ...
  const slot = data.slot_name
    ? `<p><strong>Slot:</strong> ${escapeHtml(data.slot_name)}${data.slot_time ? ` · ${escapeHtml(data.slot_time)}` : ""}</p>`
    : "";
  const service = data.service_name
    ? `<p><strong>Service:</strong> ${escapeHtml(data.service_name)}</p>`
    : "";
...
function doctorInvite(data: Record<string, unknown>): RenderedEmail {
...
export async function sendTransactionalEmailServer(
  ...
  try {
    const rendered =
      opts.templateName === "booking-confirmation"
        ? bookingConfirmation(opts.templateData ?? {})
        : doctorInvite(opts.templateData ?? {});
    ...
    if (error) {
      console.error("Transactional email failed", { templateName: opts.templateName, error });
